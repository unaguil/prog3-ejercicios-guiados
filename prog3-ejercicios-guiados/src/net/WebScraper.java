...
/**
 * Proporciona un método de utilidad para descargar la descripción de una
...
public class WebScraper {
	 ...
	 * 
	 * @param disciplineURL url de la página donde se encuentra la descripción
	 * @return devuelve la lista de párrafos que contienen la descripción de la
	 *         disciplina.
	 * @throws WebScraperException si ocurre un error al descargar la descripción
	 */
	public static List<String> getDescription(URL disciplineURL) throws WebScraperException {
		// creamos el cliente HTTP para hacer la petición a la web
		HttpClient client = HttpClient.newBuilder().version(Version.HTTP_1_1).build();

		System.out.println("Descargando la descripción desde " + disciplineURL);
		try {
			HttpRequest request = HttpRequest.newBuilder(disciplineURL.toURI()).build();
			...
			throw new WebScraperException("URL incorrecta: " + disciplineURL, e);
		} catch (IOException e) {
			throw new WebScraperException("Error al hacer la petición HTTP", e);
		} catch (InterruptedException e) {
			throw new WebScraperException("Petición HTTP interrumpida", e);
		}
	}
